...
runBeam files fun args = do
  ...
  let (at, emods) = foldr (\beam (at,emods) -> let (emod, at', beam') = beamToModule beam at in (at', emod:emods)) (AT.basic,[]) beams
      ...
      p = makeProcess emods at funNameAtom args
      stepper = do
        p <- get
  ...
  putStrLn $ ppShow $ pAtomTable p
  (steps, _p') <- runStateT stepper p
...
stackTrace = do
  ...
  lst <- forM stack $ \(StackFrame modName ip) -> do
    ...
    let Just (stackFrame@(mod, fun, ar)) = ipToFunction code ip
    return $ ETuple [EAtom mod, EAtom fun, EInteger (fromIntegral ar)]
...
ipToFunction code n =
  case opAtIp code n of
    FuncInfo mod fun arity -> Just (mod, fun, arity)
